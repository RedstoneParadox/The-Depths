...
class MossBlock: Block(FabricBlockSettings.copy(Blocks.OAK_LEAVES).build()) {
    ...
    override fun getStateForNeighborUpdate(state1: BlockState, direction: Direction?, state2: BlockState, iWorld: IWorld, pos1: BlockPos, pos2: BlockPos): BlockState {
        return if (!state1.canPlaceAt(iWorld, pos1)) Blocks.AIR.defaultState else super.getStateForNeighborUpdate(state1, direction, state2, iWorld, pos1, pos2)
    }
